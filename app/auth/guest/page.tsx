        ...
        <div className="flex flex-col gap-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>{t("auth.guest.description")}</CardDescription>
            </CardHeader>
            <CardContent>
                ...
                <div className="flex flex-col gap-6">
                  <div className="grid gap-2">
                    ...
                    <Input
                      ...
                      maxLength={30}
                    />
                    <p className="text-xs text-muted-foreground">{t("auth.guest.displayNameHint")}</p>
                  </div>
                  {error && <p className="text-sm text-destructive">{error}</p>}
                  <Button type="submit" className="w-full" disabled={isLoading}>
                    {isLoading ? t("auth.guest.loading") : t("auth.guest.button")}
                  </Button>
                  <div className="text-center text-xs text-muted-foreground">{t("auth.guest.temporary")}</div>
                </div>
